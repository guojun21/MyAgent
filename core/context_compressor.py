...
"""
    ...
    def auto_compact(
        ...
        print(f"原始: {len(messages)}条, 估算{self._estimate_tokens(messages)}tokens")
        ...
        # 激进压缩最近消息（如果过大）
        if recent_tokens > max_tokens * 0.4:  # 超过40%
            print(f"⚠️ 最近消息过大({recent_tokens}tokens)，激进压缩到{int(max_tokens * 0.25)}tokens...")
            recent_msgs = self._hard_compress(recent_msgs, int(max_tokens * 0.25))
            after_tokens = self._estimate_tokens(recent_msgs)
            print(f"压缩后: {after_tokens} tokens (压缩率: {after_tokens/recent_tokens*100:.1f}%)")
        
        # 压缩旧消息
        if len(old_msgs) > 0:
            old_compressed = self._hard_compress(old_msgs, int(max_tokens * 0.15))
        else:
            old_compressed = []
