...
if [ $? -eq 0 ]; then
    ...
    echo "View results: git log --format='%h %ad %s' --date=format:'%Y-%m-%d %H:%M:%S'"
else
    echo ""
    echo "Rewrite failed!"
fi
